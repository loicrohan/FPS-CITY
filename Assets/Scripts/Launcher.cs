...
public class Launcher : MonoBehaviourPunCallbacks
{
    ...
    public TMP_Text lobbyNameText, playerNameLabel;
    ...
    public LobbyButton theLobbyButton;
    private List<LobbyButton> allLobbyButtons = new List<LobbyButton>();
    ...
    public GameObject lobbyTestButton;
...
#if UNITY_EDITOR
        lobbyTestButton.SetActive(true);
#endif
    ...
    public void CreateLobby()
    {
        if (!string.IsNullOrEmpty(lobbyNameInput.text))
        {
            ...
            PhotonNetwork.CreateRoom(lobbyNameInput.text, options);
    ...
    {
        foreach (LobbyButton lb in allLobbyButtons)
        {
            Destroy(lb.gameObject);
        }
        allLobbyButtons.Clear();

        theLobbyButton.gameObject.SetActive(false);

        for (int i = 0; i < roomList.Count; i++)
        {
            if (roomList[i].PlayerCount != roomList[i].MaxPlayers && !roomList[i].RemovedFromList)
            {
                LobbyButton newButton = Instantiate(theLobbyButton, theLobbyButton.transform.parent);
                newButton.SetButtonDetails(roomList[i]);
                newButton.gameObject.SetActive(true);

                allLobbyButtons.Add(newButton);
            }
        }

    }

    public void JoinLobby(RoomInfo inputInfo)
    {
